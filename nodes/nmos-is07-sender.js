 ...
 * NMOS IS-07 Sender - Route payloads to IS-07 receivers via MQTT
 */
...
module.exports = function(RED) {
    function NMOSIS07SenderNode(config) {
        ...
        node.deviceDescription = config.deviceDescription || 'IS-07 Sender';
        node.sourceLabel = config.sourceLabel || 'Sender Source';
        node.flowLabel = config.flowLabel || 'Sender Flow';
        node.senderLabel = config.senderLabel || 'IS-07 Sender';
        node.eventType = config.eventType || 'object';
        node.mqttQos = parseInt(config.mqttQos) || 0;
        node.targetReceiverId = config.targetReceiverId || '';
        ...
        /**
         * Get MQTT topic for this sender
        ...
        const buildSourceResource = () => {
            const resource = {
                ...
                description: `IS-07 Sender Source (${node.eventType})`,
        ...
        const buildFlowResource = () => {
            const resource = {
                ...
                description: `IS-07 Sender Flow (${node.eventType})`,
                format: 'urn:x-nmos:format:data',
                tags: {},
        ...
        const buildSenderResource = () => {
            const resource = {
                ...
                description: `IS-07 MQTT Sender - ${node.senderLabel}`,
                flow_id: node.flowId,
                ...
                device_id: node.deviceId,
                manifest_href: `http://${localIP}:${node.registry.httpPort || 1880}/x-nmos/sender/${node.senderId}/transportfile`,
        ...
        const buildGrainMessage = (payload) => {
            ...
            if (Array.isArray(payload)) {
                // Already in grain data format
                grainData = payload;
            } else if (typeof payload === 'object' && payload !== null && payload.path !== undefined) {
                // Single path/value object
                grainData = [{
                    path: payload.path,
                    pre: payload.pre !== undefined ? payload.pre : null,
                    post: payload.post !== undefined ? payload.post : payload.value
                }];
            } else {
                // Generic payload - wrap as single value
                grainData = [{
                    path: 'value',
                    pre: null,
                    post: payload
            ...
            try {
                ...
                if (action === 'get_state') {
                    msg.payload = {
                        ...
                        targetReceiverId: node.targetReceiverId,
                ...
                // If no special action, treat as data to publish
                const payloadToPublish = action ? msg.payload : msg.payload;
                const success = publishPayload(payloadToPublish);
                
                if (success) {
                    // Pass through the message with added metadata
                    msg.sender_id = node.senderId;
                    msg.source_id = node.sourceId;
                    msg.flow_id = node.flowId;
                    msg.published = true;
                    node.send(msg);
                }
